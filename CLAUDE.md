...
- **Game state** (`src/game/`): `rogue::Game`, reached through `game()`, gathers the former globals group by group (phase 5; so far `options`, `message`, `turn`, `player`, `playing` and `noscore`). The legacy macros `hero`, `pstats`, `pack`, `proom` and `max_hp` expand to `game().player.body`. The rest still live in `extern.cpp`/`init.cpp`.
- **Headers**:
