...
- **Game state** (`src/game/`): `rogue::Game`, reached through `game()`, gathers the former globals group by group (phase 5; so far `options`, `message`, `turn`, `playing` and `noscore`). The rest still live in `extern.cpp`/`init.cpp`.
- **Headers**:
