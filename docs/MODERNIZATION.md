...
- Saves are JSON since 8.3 (`persistence/SaveGame`). A new field in `Game` or the structs it holds trips the size `static_assert`s in `SaveGame.cpp`: save and load it, then update the size. Test a change to saving with the resume-equivalence replay of 8.3d.
