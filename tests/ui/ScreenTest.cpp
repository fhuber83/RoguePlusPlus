...
TEST(Screen, PutWritesAtCursorAndAdvances)
{
	Screen s;
	s.set_cursor(3, 5);
	s.put(0x01, rogue::ui::Style{rogue::ui::Color::Yellow});
	EXPECT_EQ(s.at(3, 5), (Cell{0x01, rogue::ui::Style{rogue::ui::Color::Yellow}, false}));
	EXPECT_EQ(s.row(), 3);
	EXPECT_EQ(s.col(), 6);
}
...
{
	Screen s;
	s.set_style(rogue::ui::Standout);
	s.put("ab");
	EXPECT_EQ(s.at(0, 0), (Cell{'a', rogue::ui::Standout, false}));
	EXPECT_EQ(s.at(0, 1), (Cell{'b', rogue::ui::Standout, false}));
	EXPECT_EQ(s.col(), 2);
}
...
TEST(Screen, LineDrawsLineCellsAndKeepsCursor)
{
	Screen s;
	s.set_cursor(10, 10);
	s.set_style(rogue::ui::Style{rogue::ui::Color::Green});
	s.line(0, Screen::Cols - 3, 0xc4, 10, false); // clipped at the right edge
	s.line(Screen::Rows - 2, 0, 0xb3, 10, true);  // clipped at the bottom
	EXPECT_EQ(s.at(0, Screen::Cols - 1), (Cell{0xc4, rogue::ui::Style{rogue::ui::Color::Green}, true}));
	EXPECT_EQ(s.at(0, Screen::Cols - 4), Cell{});
	EXPECT_EQ(s.at(Screen::Rows - 1, 0), (Cell{0xb3, rogue::ui::Style{rogue::ui::Color::Green}, true}));
	EXPECT_EQ(s.row(), 10);
	EXPECT_EQ(s.col(), 10);
}
...
TEST(Screen, TerminalMirrorsEveryWrite)
{
	...
	s.set(9, 9, Cell{'!', rogue::ui::Style{rogue::ui::Color::Red}, false});
	Screen::Snapshot shot = s.snapshot();
	s.erase();
